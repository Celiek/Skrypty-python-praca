...
def przetworz_plik_xlsx(
    ...
    *,
    ...
    group_cols = ["__grp_key", "__data_str", "__netto_gr", "__vat_gr", "__brutto_gr"]
    ...
    per_invoice = (
        ...
        .agg(
            ...
            cnt_rows=("__grp_key", "count"),
            brutto_gr=("__brutto_gr", "first"),
            vat_gr=("__vat_gr", "first"),
            netto_gr=("__netto_gr", "first"),
            __doc_no_norm=("__doc_no_norm", "first"),
        )
    )

    per_invoice["faktura_id"] = (
            per_invoice["__grp_key"] + "_" +
            per_invoice["data_max"] + "_" +
            per_invoice["brutto_gr"].astype(str)
    )

    dups = per_invoice.duplicated(subset=["__grp_key", "data_max", "brutto_gr"])
    if dups.any():
        print("[WARN] Powielone wpisy (potencjalne błędne agregacje):")
        print(per_invoice.loc[dups, ["__grp_key", "data_max", "brutto_gr"]])

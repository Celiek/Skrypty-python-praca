...
def update_xml(input_file, output_file):
    ...
    # WCZYTANIE XLSX
    # ======================================================
    df = pl.read_excel(input_file, dtype=str).fillna("")
...
class App(tk.Tk):
    def __init__(self):
        ...
        self.title("Zamiania excell na xml")
        self.geometry("540x300")
        self.file_path = None

        tk.Button(self,text = "Wrzuć tutaj plik excella",command=self.choose_file,
    ...
    def process_file(self):
        if not self.file_path:
            messagebox.showerror("Błąd","Najpierw wybierz plik XLSX...")
            return

...
def wczytaj_plik():
    ...
    insp = inspect(engine)
    mcols = {c["name"] for c in insp.get_columns("merchanci")}
    scols = {c["name"] for c in insp.get_columns("merchanci_staging")}
    HAVE_REL = ("relation_status" in mcols) and ("relation_status" in scols)

    # --- kolumny wymagane wg schematu ---
    required_cols = [
    "id","nazwa","status","data_utworzenia","merchant_mail","nip","merchant_id","merchant_adres",
    "merchant_mail_fv","telefon_merchant","regulations_acceptance_date"
] + (["relation_status"] if HAVE_REL else [])
    ...
    with engine.begin() as conn:
        ...
        # wybór klucza do porównania: preferuj NIP, inaczej ID
        if "nip" in mcols and "nip" in scols:
            key = "nip"
        elif "id" in mcols and "id" in scols:
            key = "id"
        else:
            raise RuntimeError("Brak wspólnego klucza (nip/id) między staging a merchanci.")

        # kolumny do INSERTU = część wspólna tabel – bez pól, których nie chcemy kopiować
        exclude_cols = {"data_aktualizacji", "next_meeting_date", "held_first_meeting", "relation_status"} - ({""} if HAVE_REL else {"relation_status"})
        insert_cols = sorted((mcols & scols) - exclude_cols)

        # upewnij się, że klucz jest na liście
        if key not in insert_cols:
            insert_cols.append(key)

        cols_list    = ", ".join(insert_cols)
        select_list  = ", ".join(f"r.{c}" for c in insert_cols)
        ...
        INSERT INTO merchanci ({cols_list})
        SELECT {select_list}
        FROM ranked r
        LEFT JOIN merchanci m
        ON m.{key} = r.{key}
        WHERE r.rn = 1
        AND m.{key} IS NULL
        """)
        ...
        with engine.begin() as con:
            result = con.execute(sql_only_new)
            try:
                ins_count = result.rowcount
            except Exception:
                ins_count = None
            logger.info("Wstawiono nowych rekordów z staging: %s", ins_count)
